using System;

class BankAccount
{
    //04.A bank account has a holder name (first name, middle name and last name), 
    //available amount of money (balance), bank name, IBAN, BIC code and 3 credit card numbers 
    //associated with the account. Declare the variables needed to keep the information for a 
    //single bank account using the appropriate data types and descriptive names.

    static void Main(string[] args)
    {
        //Holder info
        string holderFirstName = "Pesho";
        string holderMiddleName = "Georgiev";
        string holderLastName = "Goshov";

        decimal balance = 0.0m;

        string bankName = "NoName Bank";

        string IBAN = "212usu1210221312321";
        string bicCode = "UNSSL";

        string firstCardNumber = "";
        string secondCardNumber = "";
        string thirdCardNumber = "";

    }
}

